...
- **Read an issue**: `gh issue view <number> --json title,body,comments,labels`. Prefer `--json` over `--comments`: an empty `comments` array unambiguously means no comments, while empty `--comments` output cannot be told apart from a render failure — and a silently missed comment is a silently missed spec line.
...
Run `gh issue view <number> --json title,body,comments,labels`.
