...
- Do not overwrite an existing branch; if the proposed name exists, increment its trailing `-<N>` suffix.
- The PR base is the branch `/build` was invoked from.
- Open a draft PR; do not merge.

## Loop

1. Identify the parent spec and child tickets from the user's input. If only a parent spec is given, find its child issues using the conventions in `REFERENCE.md` and confirm the batch with the user. If inputs are missing, ask.
...
See [`REFERENCE.md`](REFERENCE.md) for branch naming, review classification, PR body, diff-size guard, and examples.
