...
5. **Final review.** Run `/review` with `REVIEW_BASE` = `PRE_BUILD_SHA`, `REVIEW_HEAD` = the batch tip, `SPEC_SOURCE` = the parent spec (body and comments) plus every seam's `Watch items` verbatim, `CARRIED` = the run's settled findings per `## Per-ticket review`, `AXES=both`, `REPORT_DIR` = the run's `reports/` so each axis self-persists and returns a digest, `SUITE_RESULT` = the latest batch-branch `dotnet test` result. Fixes dispatch back through an implementer; the ticket branch is already merged, so fixes land as new commits — never amend or rebase the merged tip. Act on `REVIEW_STATUS` per `/review`'s output contract.
   **Done when:** `REVIEW_STATUS` is `clean`, `resolved`, or `deferred`, or the maintainer has been consulted on `escalated`/`capped`.

6. **Close out.** Confirm the pushed branch's checks are green on the self-hosted runner — Release-config tests, installer package, clean checkout; CI is the end-of-run gate, not a per-merge loop. Finalize the draft PR body per `docs/agents/pr-conventions.md` `## PR body` with the full `REVIEW_NOTES` under `### Review notes`. Remove the `.worktrees/` worktrees and delete the `afk/<ticket>` branches once the PR is open — stale `afk/` branches false-positive the prior-work check on later runs. Write each ticket's terminal state to `STATUS.json` — `phase`, `status`, `commit`, `pr`, `fix_round`, `review_status`, `ci` — before dispatching the retro, which reads this file and must see settled state. Dispatch the run retro — and the carried-list triage when the list is non-empty — per `REFERENCE.md` `## Run retro` / `## Carried-list triage`; both run background and overlap closeout. Deliver the run summary with the PR link, the triaged carried list (each item marked `nit` / `watch` / `reopen`, with its reason and revisit condition), and the top retro candidates, then hand off to `/qa`.
   **Done when:** checks are green, the draft PR body is complete, terminal `STATUS.json` state written, worktrees removed and `afk/` branches deleted, `reports/run-retro.md` — and `reports/carried-triage.md` when the triage dispatched — is persisted, and the summary is delivered.
