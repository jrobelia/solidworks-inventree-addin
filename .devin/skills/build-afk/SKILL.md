...
- Dispatch is serial and foreground by default. Once session tool grants exist it may fan out per `REFERENCE.md` `## Dispatch mechanics` — designers to 5, background implementers to 3 (5 across an independent set). A backgrounded subagent auto-denies every tool it is not pre-granted, so pre-approve each profile's tools before dispatching it in the background: `exec` for implementers and reviewers (a backgrounded reviewer without the grant stalls on its first `git diff`), the designer's read/grep/glob set. Interrupting the session parks subagents rather than killing them; they resume on the next message.
...
4. **Implement → merge → review.** Pick the riskiest unblocked ticket first (architectural seam, integration point, unknowns), ties by issue order. For each ticket, per `REFERENCE.md` `## Dispatch mechanics` (independent tickets may overlap) and `## Per-ticket review`:
   ...
   - Fill `IMPLEMENTER_TASK.md` and dispatch `build-implementer`.
