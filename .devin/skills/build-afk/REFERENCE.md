...
- Foreground is the default. A backgrounded subagent auto-denies ungranted tools: pre-approve `exec` in-session before dispatching anything in the background — designers and `review-spec` included, not just implementers (a backgrounded reviewer re-runs the test suite to verify claims and stalls on the denial) — and cap background implementers at 2. When a denial stalls a background subagent, foreground it from the subagent panel (`f` on the running entry) or resume it — resume always runs foreground.
