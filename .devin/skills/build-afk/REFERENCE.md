...
- `STATUS.json` — machine-readable state: batch branch, `PRE_BUILD_SHA`, per-ticket `{phase, status, branch, worktree, commit, fix_round, blocked_kind, pr, review_status, ci}` plus `entered_at` — an ISO-8601 stamp recording when the current phase began, validated to parse on write (a corrupt stamp defeats the dispatch-gap queries it exists for); the full transition trail lives in `PROGRESS.md`'s timestamped entries, so dispatch-to-artifact gaps are queryable without git archaeology. Terminal fields (`commit`, `pr`, `review_status`, `ci`, final `phase`/`status`) land at closeout, before the retro dispatch — the retro reads this file, and a mid-flight snapshot reports stale state as a tooling gap.
- `PROGRESS.md` — the ledger. First line names the spec. Every entry carries an `HH:MM` local (or ISO-8601) timestamp prefix — merges, review verdicts, fix rounds, gate decisions, dispatch attempts including failed ones (a recovered dispatch is indistinguishable from a clean one without them); a timestamp on write, no timing machinery — a real stamp, never a placeholder: the field exists to make dispatch-to-artifact gaps queryable. `Task <N>: complete` per merged ticket. A trailing `Task <N>: fix round <R>` line means resume mid-ladder at round `R+1`.
