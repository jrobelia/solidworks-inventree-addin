...
- Run a smoke test pass from [CHECKLIST.md](CHECKLIST.md) at the start of the walk, before the issue-specific groups. Order the smoke tests by risk based on the diff. The engineer can skip any smoke test, but the agent should recommend the full set. This catches environment or load-path regressions the focused steps may miss.
