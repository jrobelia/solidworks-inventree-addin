...
1. **Resolve inputs.** `git rev-parse REVIEW_BASE` must resolve and `git diff REVIEW_BASE...<REVIEW_HEAD or HEAD>` must be non-empty — a bad ref or empty diff fails here, not inside subagents. If `SPEC_SOURCE` is an issue number, fetch the full body and **all comments** per `docs/agents/issue-tracker.md` `## Comments are part of the spec`; if it is a path, read the file.
