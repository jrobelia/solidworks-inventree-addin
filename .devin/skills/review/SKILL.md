...
3. **Dispatch the axes.** Preferred: `run_subagent` with profiles `review-standards` and `review-spec` in parallel (`is_background=true`), passing `REVIEW_BASE`; pass the `SPEC_SOURCE` contents to the Spec axis as its `SPEC:` block. `SPEC_SOURCE: none` skips the Spec axis and records "no spec available". Fallback: if `run_subagent` is unavailable or denied, run `subagent_general` in the foreground — read the profile file from `.devin/agents/` and use its full text as the task, appended with the same inputs.
4. **Aggregate.** Collect the `## Standards` and `## Spec` blocks verbatim and report the axes separately.
