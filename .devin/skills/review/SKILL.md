...
8. **Return** `REVIEW_STATUS` and `REVIEW_NOTES`. Notes must list each finding's disposition (fixed / deferred-with-reason / escalated-to-issue) so the PR body shows an audit trail.
