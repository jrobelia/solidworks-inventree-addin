...
description: "Per-ticket implementer for /build-afk. Runs a complete red-first TDD loop in its own git worktree — implement, verify, self-review, one commit — and returns a structured status the orchestrator routes on. Never spawns subagents, never opens a PR."
model: swe-2-high
allowed-tools:
  - read
  - write
